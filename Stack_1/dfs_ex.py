...
def dfs(start):
    ...
    while stack:
        ...
        # 현재위치: 경로상 마지막 정점
        current = stack[-1]
        # print(current,end=' ')  # 되돌아온것까지 출력
        # adj[current]를 살펴보자!
        for v in range(1, V+1): # V: current와 인접한지 확인하려는 정점 번호
            # v 정점이 current와 인접하며 아직 방문하지 않았으면 방문
            if adj[current][v] and visited[v] == 0:
            # if adj[current][v] and not visited[v]:
                stack.append(v)
                visited[v] = 1
                print(v, end=' ')
                break # 이동 가능한 경로 찾았으니 길 찾는거 멈추고 이동
        else: # current에서 이동가능 한 정점이 없음!
...
# 1 2 4 6 5/ 6 7 3/ 7 6 /4 2 1 DFS 끝!
# 되돌아온것까지 출력