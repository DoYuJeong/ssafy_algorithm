...
def shoot(n,bricks):
    ...
    if n == N: # 이미 N개의 구슬을 발사했으면
        # 구슬 다 쏴 봤으니깐 벽돌 상태 확인(남아 있는 벽돌 개수 세기)
        for i in range(H):
            for j in range(W):
                if bricks[i][j]:
                    cnt += 1
...
def bomb(col,target_bricks):
    ...
    while queue:
        ...
        for d in range(4):
            for l in range(length): # 벽돌 숫자 만큼
                nr = cr + dr[d]*l
                nc = cc + dc[d]*l
                # 벽돌이 있고, 아직 터뜨린 벽돌이 아니라면 터뜨릴 대상에 추가
    ...
    # i를 1씩 증가시키면서 벽돌이라면 j번째에 가져다 놓기
    for i in range(W):
        k = H - 1  # 벽돌을 가져다 놓을 위치(벽돌이 놓여지면 1 증가)
        for j in range(H-1,-1,-1):
            # 만약에 j번째에 벽돌이 있으면 가장 아래칸에 가져다 놓기
            if target_bricks[j][i]:
                target_bricks[j][i],target_bricks[k][i] = target_bricks[k][i],target_bricks[j][i]

    # 벽돌 연쇄 반응 끝나고 중력 처리 까지 끝난 벽돌들...
