...
# 테스트 케이스 입력 받기 T 번 반복
for tc in range(1, T+1):
    # 테스트 케이스의 첫 번째 줄에는 숫자의 개수
    # 테스트 케이스의 두 번째 줄에는 숫자들
    N = int(input())
    # number = input() # 477162 658880 751280 927930 297191
    # input() 자르고 숫자로 바꾸기
    ...
    for i in range(N): # numbers를 살펴보는 반복문
        if numbers[i] > max_v:
            max_v = numbers[i]
    min_v = 1000000    # numbers의 요소가 min_v 보다 더 작으면 min_v 값 바꾸기
    # 문제에서 나올만한 가장 큰 값을 넣어줌, 999999999
    for i in range(N):
        if numbers[i] < min_v:
            min_v = numbers[i]
